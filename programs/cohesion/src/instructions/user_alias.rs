...
pub fn create_user_alias(ctx: Context<CreateUserAlias>, alias: String) -> Result<()> {
	...
	require!(alias.chars().count() <= 50, ErrorMessage::TooLong);
...
pub fn update_user_alias(ctx: Context<UpdateUserAlias>, new_alias: String) -> Result<()> {
	...
	require!(alias.alias != new_alias, ErrorMessage::NothingChanged);
	require!(alias.alias.chars().count() <= 50, ErrorMessage::TooLong);
